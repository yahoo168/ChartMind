...
class ContentDAO(MongodbBaseDAO, Generic[T]):
    ...
    @ensure_initialized
    async def full_text_search(self, query_text, limit=10, user_id=None, min_score=0.5, access_filter=None):
        """全文搜索方法"""
        return await super().full_text_search(query_text, limit, user_id, min_score, access_filter)
    
    @ensure_initialized
    async def vector_search(self, query_vector, limit=10, user_id=None, num_candidates=100, min_score=0, access_filter=None):
        """向量搜索方法"""
        return await super().vector_search(query_vector, limit, user_id, num_candidates, min_score, access_filter)
